...
import { trashGroup, updateGroup } from "@/server-actions/group";
...
export default function GroupCard({
      ...
      setGroups((prev) => {
        prev.groups.forEach((group) => {
          if (group.group_id === serverMessage_updateGroup.result?.group_id) {
            ...
            group.description = serverMessage_updateGroup.result.description;
  ...
  const handleTrashGroup = async (group_id: string) => {
    ...
    if (serverMessage_trashGroup.status === "success") {
      setGroups((prev) => {
        prev.groups = prev.groups.filter(
            ...
            <TooltipProvider delayDuration={20}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <p className="max-w-[200px] truncate">{g.description}</p>
                </TooltipTrigger>
                <TooltipContent>
                  <p>{g.description}</p>
                </TooltipContent>
      ...
      <ContextMenuContent>
        <Dialog
          ...
          onOpenChange={(e) => {
            ...
            form.setValue("description", g.description || "");
          }}
        >
        ...
        <Dialog
          ...
          onOpenChange={(e) =>
            setDialogState({ ...dialogState, trash_group: e })
          }
        >
          <DialogTrigger asChild>
            <ContextMenuItem onSelect={(e) => e.preventDefault()}>
